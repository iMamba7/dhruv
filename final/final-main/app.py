...
def view_data(company):
    ...
    if request.method == 'POST':
        ...
        if data:
            # Define the headers
            headers = ['Date', 'Name', 'Box', 'Dozen', 'Total Items', 'Weight', 'Total Weight']
            column_name = ['Date', 'Name', 'Box', 'Dozen', 'Total Items', 'Weight', 'Total Weight']

            if selection == 'paisa':
                column_name = ['Date', 'Name', 'Box', 'Dozen', 'Total Items', 'paisa', 'Total paisa']

            # Convert to list of dictionaries
            data = [dict(zip(headers, row)) for row in data]
            
            # Filter out rows where both Weight and Total Weight are None
            data = [
